function colorscale = CBIG_GenerateFSColorscale(res, min_val, max_val, output_dir)

% colorscale = CBIG_GenerateJetColorscale(res, min_val, max_val, output_dir)
%
% Produce a discretized colorscale based on Matlab's jet colorscale
%
% Input:
...
% Output:
%   - colorscale: colorscale in Matlab's format with res number of values discretized from the jet color scheme
%
% colorscale = CBIG_GenerateJetColorscale(28, 5e-5, 1, ...
%   '/Work/data/colorscale')
%   Generate a discrete colorscale of 28 bins with the value ranging from
%   5e-5 to 1. The colors come from the Matlab's jet color pallete
%
% Written by Nanbo Sun and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md

    [~,~, colorscale] = read_fscolorlut(fullfile(getenv('FREESURFER_HOME'),'SegmentNoLUT.txt'));

    colorscale = colormap(colorscale(1:res,1:3)/255);
